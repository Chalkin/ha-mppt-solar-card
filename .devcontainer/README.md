...
3. **Build the Card**
   ```bash
   pnpm build      # Lint and build
   pnpm start      # Start dev server with hot reload (port 5000)
   pnpm lint       # Check code quality
   pnpm rollup     # Production build
   ```
...
pnpm start              # Runs Rollup in watch mode on port 5000

# Quality checks
pnpm lint             # ESLint check
pnpm build            # Full build pipeline (lint + rollup)

# Production build
pnpm rollup           # Create optimized dist files
```
...
- All pnpm commands run inside the container automatically
