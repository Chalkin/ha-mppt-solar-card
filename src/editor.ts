...
import { MpptSolarCardConfig } from './types';
import { customElement, property, state } from 'lit/decorators.js';

@customElement('mppt-solar-card-editor')
export class MpptSolarCardEditor extends LitElement implements LovelaceCardEditor {
  @property({ attribute: false }) public hass!: HomeAssistant;

  @state() private _config?: MpptSolarCardConfig;

  @state() private _openSection = 'entity';

  private _configValueTarget(
    ev: Event,
  ): (EventTarget & { configValue?: keyof MpptSolarCardConfig; value?: string; checked?: boolean }) | null {
    return ev.target as EventTarget & {
      configValue?: keyof MpptSolarCardConfig;
      value?: string;
      checked?: boolean;
    };
  }
      ...
      ${this._renderSection(
        ...
        html`
          ...
          <ha-selector
            ...
            .value=${this._config.chart_height ?? 64}
  ...
  private _selectorChanged(ev: CustomEvent): void {
    ...
    const configValue = target.configValue as keyof MpptSolarCardConfig;
    if (!configValue) {
      return;
    }
    this._config = {
  ...
  private _actionChanged(ev: CustomEvent): void {
    ...
    if (newAction && newAction.action) {
      updatedConfig[configValue as keyof MpptSolarCardConfig] = newAction;
    } else {
      ...
      updatedConfig[configValue as keyof MpptSolarCardConfig] = defaults[configValue as keyof typeof defaults] || {
        action: 'none',
      };
    }
...
if (!customElements.get('mppt-solar-card-editor')) {
  customElements.define('mppt-solar-card-editor', MpptSolarCardEditor);
}
